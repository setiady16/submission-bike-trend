...
import numpy as np # Import numpy untuk np.where jika capping outlier digunakan
import os
...
# Saya asumsikan saat ini 'main_data.csv' berada di dalam subfolder 'dashboard' relatif terhadap tempat script dijalankan
...
def clean_data(df_raw):
    ...
    # 4. Handle Missing Values (jika ada, tambahkan logika sesuai notebook Anda)
    # Contoh: if df_cleaned.isnull().sum().any():
    ...
    #             # Pastikan ini konsisten dengan penanganan di notebook Anda

    # 5. Handle Outliers (sesuai keputusan di notebook Anda)
    # Contoh: Jika Anda melakukan capping pada 'windspeed'
...
start_date = df['dteday'].min().date() # Ambil hanya tanggal
end_date = df['dteday'].max().date() # Ambil hanya tanggal
selected_date = st.sidebar.date_input(
    "Pilih Rentang Tanggal", [start_date, end_date], min_value=start_date, max_value=end_date
)

# Konversi selected_date ke datetime
...
# Mapping Hari dalam Seminggu untuk Tampilan
weekday_mapping = {
...
# --- TAMPILKAN DATA (Optional, untuk debugging/verifikasi) ---
...
# 1.3 Jumlah Pengguna Berdasarkan Musim (seasons)
...
# 1.4 Jumlah Pengguna Berdasarkan Kondisi Cuaca (weathersit)
...
st.markdown("""
- **Pola Musiman:** Penggunaan sepeda (total, terdaftar, dan kasual) menunjukkan pola musiman yang jelas, dengan puncak penyewaan terjadi pada **Musim Panas** dan **Musim Gugur**. Bulan-bulan ini kemungkinan besar memiliki kondisi cuaca yang paling ideal untuk bersepeda. Sebaliknya, **Musim Dingin** dan **Musim Semi** awal menunjukkan jumlah penyewaan yang jauh lebih rendah.
